...
>
> **Re-sequenced 2026-06-19 (operator directive):** tool-funnel blogs are the highest-converting content, so they are pulled forward. New order after Phase 25 (Local SEO): Phase 26 = **Tool Coverage** (one post per uncovered free tool, 12 blogs, each linking its tool), then the remaining ~42 slots distributed across pillars 4-8 + technical to land exactly on 87. Total stays 87 (full 29-day window). `blog:coverage --strict` (every public tool linked from a published post) must pass before v9 closes; see the tool-coverage table in `.planning/CONTENT-STRATEGY.md`.
