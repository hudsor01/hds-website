...
>
> **Closing.** Audit in flight at `.planning/milestones/v5-AUDIT.md`. 4/5 phases shipped (06-09) + 1 cross-phase hotfix (PR #226). Phase 10 scoped into this audit cycle on operator override (defensive-only deferral overridden); v5 closes once Phase 10 ships.
...
| 10 | `admin-list-pagination` | planning (in flight) | 0 | Add server-rendered cursor pagination + a text-search filter to the 7 admin list pages. Currently capped at the hard limits set in Phase 04/05 (showcase 50, blog 50, testimonials 50, leads 200, calculator-leads 200, newsletter 200, emails 100). Originally defensive-scoped, **scoped INTO v5 close 2026-05-27 on operator override** -- no table currently exceeds 25% of cap, but the search-by-text capability is needed against the ops tables (leads, newsletter, emails) regardless of row count. v5 closes once this phase ships. |
