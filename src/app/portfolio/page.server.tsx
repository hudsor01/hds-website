...
// Server-side data fetching for portfolio projects
async function getPortfolioProjects(): Promise<PortfolioProject[]> {
  try {
    // In a real implementation, the API would be at an external URL with proper authentication
    // const res = await fetch(`${process.env.API_BASE_URL}/api/portfolio/projects`, {
    //   headers: {
    ...
    //
    // if (!res.ok) {
    //   console.error('Failed to fetch portfolio projects:', res.status, res.statusText);
    //   throw new Error('Failed to fetch portfolio projects');
    // }
    // 
    // return res.json();

    // For demo purposes, we'll simulate the API call with static data
    // This simulates what would come from an actual API
