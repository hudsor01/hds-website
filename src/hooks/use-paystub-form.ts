...
import type { PayFrequency, PaystubData } from '@/types/paystub'
...
export function usePaystubForm() {
	const [paystubData, setPaystubDataState] =
		useState<PaystubData>(INITIAL_PAYSTUB)
	...
	const setPaystubData = (
		update: Partial<PaystubData> | ((prev: PaystubData) => PaystubData)
	) => {
		setPaystubDataState(prev =>
			typeof update === 'function'
				? (update as (prev: PaystubData) => PaystubData)(prev)
				: { ...prev, ...update }
		)
	}
