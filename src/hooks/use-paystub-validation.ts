...
export function usePaystubValidation({
	...
	const mapZodIssuesToFormErrors = (
		issues: ReturnType<typeof paystubFormSchema.safeParse>['error'] | undefined
	) => {
	...
	const validateForm = () => {
		...
		const combinedErrors: FormErrors = {
			...mapZodIssuesToFormErrors(
				baseValidation.success ? undefined : baseValidation.error
			)
		}
