...
let scheduledEmailsQueue: ScheduledEmail[] = [];
 ...
 */
async function sendScheduledEmail(scheduledEmail: ScheduledEmail): Promise<void> {
  if (!resend) {
  ...
  try {
    ...
    const processedSubject = processEmailTemplate(step.subject, scheduledEmail.variables);
    const processedContent = processEmailTemplate(step.content, scheduledEmail.variables);
      ...
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; line-height: 1.6; color: #333;">
        ${processedContent.split('\\n\\n').map(paragraph => 
          paragraph.startsWith('**') && paragraph.endsWith('**') 
            ? `<h3 style="color: #0891b2; margin: 25px 0 15px 0;">${escapeHtml(paragraph.slice(2, -2))}</h3>`
            : paragraph.startsWith('• ') 
            ? `<li style="margin: 8px 0;">${escapeHtml(paragraph.slice(2))}</li>`
            : paragraph.includes('• ')
            ? `<ul style="margin: 15px 0; padding-left: 20px;">${paragraph.split('\\n').filter(line => line.startsWith('• ')).map(item => 
              `<li style="margin: 8px 0;">${escapeHtml(item.slice(2))}</li>`).join('')}</ul>`
        ...
        <div style="margin-top: 40px; padding-top: 20px; border-top: 1px solid #e2e8f0; font-size: 14px; color: #64748b;">
          ...
          <p style="margin-top: 15px; font-size: 12px; color: #94a3b8;">
            ...
            <a href="https://hudsondigitalsolutions.com/unsubscribe?email=${encodeURIComponent(scheduledEmail.recipientEmail)}" style="color: #0891b2;">Unsubscribe</a>
          </p>
        </div>
      </div>
    `;
    ...
    console.log(`Sent scheduled email: ${processedSubject} to ${scheduledEmail.recipientEmail}`);

  } catch (error) {
    scheduledEmail.status = 'failed';
    scheduledEmail.error = error instanceof Error ? error.message : 'Unknown error';
    
    console.error(`Failed to send scheduled email to ${scheduledEmail.recipientEmail}:`, error);
 ...
 */
export function cancelEmailSequence(recipientEmail: string, sequenceId?: string): void {
  scheduledEmailsQueue = scheduledEmailsQueue.filter(email => 
    !(email.recipientEmail === recipientEmail && 
      email.status === 'pending' && 
      (!sequenceId || email.sequenceId === sequenceId))
  );
  
  console.log(`Cancelled scheduled emails for ${recipientEmail}${sequenceId ? ` in sequence ${sequenceId}` : ''}`);
}
 ...
 */
export async function processEmailsEndpoint(): Promise<{ success: boolean; processed: number; errors: number }> {
