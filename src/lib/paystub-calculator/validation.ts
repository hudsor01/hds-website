...
export const validateEmployeeInputs = (employeeData: {
	...
	if (
		...
		employeeData.employeeName.trim().length < 2
	) {
	...
	if (
		employeeData.hoursPerPeriod !== undefined &&
		employeeData.hoursPerPeriod <= 0
	) {
		errors.hoursPerPeriod = 'Hours per pay period must be greater than 0'
	}
...
export const validatePayPeriodInputs = (
	grossPay: number,
	filingStatus: string,
	state: string
) => {
	...
	if (grossPay <= 0) {
		...
		// $100,000 per pay period is extremely high
