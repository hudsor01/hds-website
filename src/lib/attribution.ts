...
import type { LeadAttributionData, UTMParameters } from '@/types/analytics'
...
function determineTrafficSource(
	...
	if (searchEngines.some(engine => referrerDomain.includes(engine))) {
		return {
			source:
				searchEngines.find(engine => referrerDomain.includes(engine)) ||
				'search',
			medium: 'organic'
		}
	}
	...
	if (socialPlatforms.some(platform => referrerDomain.includes(platform))) {
		return {
			source:
				socialPlatforms.find(platform => referrerDomain.includes(platform)) ||
				'social',
			medium: 'social'
		}
	}
