import type { LocationData } from './types'

export const ARKANSAS_LOCATIONS: LocationData[] = [
	{
		slug: 'little-rock',
		city: 'Little Rock',
		state: 'Arkansas',
		stateCode: 'AR',
		tagline: 'Web Design for Little Rock Small Businesses',
		description:
			"As Arkansas's capital and largest city, Little Rock spans Downtown, the River Market and Hillcrest. I design and build fast, mobile-ready websites for Rock City small businesses.",
		metaDescription:
			'Web design in Little Rock, AR for small businesses. I build fast, mobile-friendly websites for shops, clinics and firms from Downtown to the River Market.',
		neighborhoods: [
			'Downtown',
			'River Market',
			'Hillcrest',
			'The Heights',
			'West Little Rock',
			'North Little Rock',
			'Maumelle',
			'Sherwood'
		],
		stats: {
			businesses: '10+',
			projects: '40+',
			satisfaction: '100%'
		},
		features: [
			{
				title: 'Local Business Websites',
				description:
					'Clean, mobile-ready websites for Little Rock shops, restaurants and service businesses that turn visitors into customers.'
			},
			{
				title: 'Healthcare Practice Sites',
				description:
					'Professional websites for Little Rock clinics, dental offices and medical practices that help new patients find and contact you.'
			},
			{
				title: 'Local SEO Setup',
				description:
					'Websites built to rank on Google so Little Rock customers searching for your services find your business first.'
			}
		]
	},
	{
		slug: 'fort-smith',
		city: 'Fort Smith',
		state: 'Arkansas',
		stateCode: 'AR',
		tagline: 'Web Design for Fort Smith Small Businesses',
		description:
			'Fort Smith anchors the Arkansas River Valley with a rich frontier heritage and a hard-working small business community. I design and build modern websites for Fort Smith businesses.',
		metaDescription:
			'Web design in Fort Smith, AR. I build fast websites for Arkansas River Valley small businesses, from Midland Boulevard trades to downtown shops.',
		neighborhoods: [
			'Downtown',
			'Midland Boulevard',
			'Fianna Hills',
			'Van Buren',
			'Greenwood',
			'Alma',
			'Barling',
			'Lavaca'
		],
		stats: {
			businesses: '10+',
			projects: '40+',
			satisfaction: '100%'
		},
		features: [
			{
				title: 'Fast, Modern Websites',
				description:
					'Quick-loading, mobile-first websites for Fort Smith contractors, trades and family-owned businesses.'
			},
			{
				title: 'Healthcare Practice Sites',
				description:
					'Easy-to-navigate websites for Fort Smith medical, dental and wellness practices that bring in new patients.'
			},
			{
				title: 'Tourism & Heritage Sites',
				description:
					'Inviting websites for Fort Smith historic sites, museums and Arkansas River Valley tourism businesses.'
			}
		]
	},
	{
		slug: 'fayetteville',
		city: 'Fayetteville',
		state: 'Arkansas',
		stateCode: 'AR',
		tagline: 'Web Design for Fayetteville Small Businesses',
		description:
			'Home to the University of Arkansas and a lively Dickson Street scene, Fayetteville is one of the fastest-growing metros in the country. I design and build websites for Fayetteville small businesses.',
		metaDescription:
			'Web design in Fayetteville, AR. I build fast, mobile-ready websites for small businesses around the Downtown Square, Dickson Street and the U of A campus.',
		neighborhoods: [
			'Downtown Square',
			'Dickson Street',
			'Gulley Park',
			'Wedington',
			'Farmington',
			'Elkins',
			'Greenland',
			'West Fork'
		],
		stats: {
			businesses: '10+',
			projects: '40+',
			satisfaction: '100%'
		},
		features: [
			{
				title: 'Local Business Websites',
				description:
					'Mobile-first websites for Fayetteville shops, cafes and service businesses, with booking and contact forms built in.'
			},
			{
				title: 'E-Commerce Websites',
				description:
					'Online stores for NWA cycling companies, outdoor gear brands and trail tourism businesses around Fayetteville.'
			},
			{
				title: 'Campus-Area Sites',
				description:
					'Polished websites for businesses near the University of Arkansas that need to reach students, staff and visitors.'
			}
		]
	},
	{
		slug: 'springdale',
		city: 'Springdale',
		state: 'Arkansas',
		stateCode: 'AR',
		tagline: 'Web Design for Springdale Small Businesses',
		description:
			"Springdale sits at the heart of Northwest Arkansas with a diverse, fast-growing business community from Har-Ber Meadows to Tontitown. I design and build websites for Springdale's small businesses.",
		metaDescription:
			'Web design in Springdale, AR. I build fast, mobile-ready websites for the diverse small businesses of Northwest Arkansas, from Tontitown to Har-Ber Meadows.',
		neighborhoods: [
			'Downtown',
			'Har-Ber Meadows',
			'Elm Springs',
			'Tontitown',
			'Bethel Heights',
			'Johnson',
			'Lowell',
			'Cave Springs'
		],
		stats: {
			businesses: '10+',
			projects: '40+',
			satisfaction: '100%'
		},
		features: [
			{
				title: 'Local Business Websites',
				description:
					'Mobile-ready websites for Springdale food businesses, agricultural suppliers and family-run shops.'
			},
			{
				title: 'Multilingual Websites',
				description:
					"Websites built for Springdale's diverse community, including Hispanic and Marshallese-owned businesses that serve customers in more than one language."
			},
			{
				title: 'Local SEO Setup',
				description:
					'Websites tuned to rank on Google so Springdale customers find your business when they search nearby.'
			}
		]
	},
	{
		slug: 'jonesboro',
		city: 'Jonesboro',
		state: 'Arkansas',
		stateCode: 'AR',
		tagline: 'Web Design for Jonesboro Small Businesses',
		description:
			'The largest city in northeast Arkansas, Jonesboro is a regional center for healthcare, education and agriculture. I design and build modern websites for Jonesboro small businesses.',
		metaDescription:
			'Web design in Jonesboro, AR. I build fast, mobile-friendly websites for northeast Arkansas small businesses in healthcare, agriculture and the ASU area.',
		neighborhoods: [
			'Downtown',
			'Indian Mall Area',
			'Hilltop',
			'Southwest Drive',
			'Brookland',
			'Bay',
			'Lake City',
			'Paragould'
		],
		stats: {
			businesses: '10+',
			projects: '40+',
			satisfaction: '100%'
		},
		features: [
			{
				title: 'Healthcare Practice Sites',
				description:
					'Clear, mobile-ready websites for Jonesboro clinics, dental offices and medical practices that help patients find and book you.'
			},
			{
				title: 'Agriculture Business Sites',
				description:
					'Straightforward websites for northeast Arkansas rice farms, agricultural suppliers and commodity businesses.'
			},
			{
				title: 'Website Redesigns',
				description:
					'Refreshing dated websites for Jonesboro businesses near ASU into fast, modern sites that look right on every device.'
			}
		]
	},
	{
		slug: 'bentonville',
		city: 'Bentonville',
		state: 'Arkansas',
		stateCode: 'AR',
		tagline: 'Web Design for Bentonville Small Businesses',
		description:
			'Home to a world-class mountain biking destination and the Crystal Bridges art scene, Bentonville punches above its weight. I design and build websites for Bentonville small businesses.',
		metaDescription:
			'Web design in Bentonville, AR. I build fast websites for small businesses near the Downtown Square, Crystal Bridges and the NWA mountain bike trails.',
		neighborhoods: [
			'Downtown Square',
			'Bella Vista',
			'Centerton',
			'Pea Ridge',
			'Decatur',
			'Gravette',
			'Highfill',
			'Little Flock'
		],
		stats: {
			businesses: '10+',
			projects: '40+',
			satisfaction: '100%'
		},
		features: [
			{
				title: 'Local Business Websites',
				description:
					'Mobile-first websites for Bentonville shops, restaurants and service businesses around the Downtown Square.'
			},
			{
				title: 'Arts & Culture Sites',
				description:
					'Visually rich websites for Bentonville art organizations, Crystal Bridges partners and cultural tourism businesses.'
			},
			{
				title: 'E-Commerce Websites',
				description:
					'Online stores for NWA cycling brands, trail companies and outdoor recreation businesses in Bentonville.'
			}
		]
	},
	{
		slug: 'hot-springs',
		city: 'Hot Springs',
		state: 'Arkansas',
		stateCode: 'AR',
		tagline: 'Web Design for Hot Springs Small Businesses',
		description:
			'Hot Springs is a national park city with a tourism-driven economy, historic Bathhouse Row and a growing arts scene. I design and build websites for Spa City small businesses.',
		metaDescription:
			'Web design in Hot Springs, AR. I build fast websites for Spa City tourism, hospitality and wellness businesses, from Bathhouse Row to Lake Hamilton.',
		neighborhoods: [
			'Downtown',
			'Bathhouse Row',
			'Lake Hamilton',
			'Hot Springs Village',
			'Jessieville',
			'Royal',
			'Fountain Lake',
			'Mountain Pine'
		],
		stats: {
			businesses: '10+',
			projects: '40+',
			satisfaction: '100%'
		},
		features: [
			{
				title: 'Tourism & Hospitality Sites',
				description:
					'Mobile-ready websites with booking and contact forms for Hot Springs hotels, bathhouses and tour operators.'
			},
			{
				title: 'Restaurant & Venue Sites',
				description:
					'Appealing websites for Hot Springs dining spots and entertainment venues near Oaklawn and Bathhouse Row.'
			},
			{
				title: 'Wellness & Spa Sites',
				description:
					'Premium, polished websites for Hot Springs spas, wellness retreats and health-focused small businesses.'
			}
		]
	}
]
