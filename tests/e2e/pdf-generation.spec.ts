...
test.describe("PDF Generation", () => {
  test.beforeEach(async ({ page }) => {
    ...
    await expect(
      page.locator("text=ANNUAL WAGE AND TAX STATEMENT")
    ).toBeVisible({ timeout: 5000 });
  });

  test("should display Save as PDF buttons on both documents", async ({
    page,
  }) => {
    ...
    await expect(pdfButtons.first()).toHaveCSS(
      "background-color",
      "rgb(5, 150, 105)"
    ); // emerald-600
  ...
  test("should trigger print dialog when PDF button is clicked", async ({
    page,
  }) => {
    // Set up print dialog listener
    await page.evaluate(() => {
      // Mock window.print to track if it was called
      (window as unknown as { originalPrint?: () => void }).originalPrint =
        window.print;
      window.print = () => {
        (window as unknown as { printTriggered?: boolean }).printTriggered =
          true;
      };
    });
    ...
    await page.evaluate(() => {
      ...
      if (win.originalPrint) {
        window.print = win.originalPrint;
      }
    });
  });

  test("should have proper document structure for printing", async ({
    page,
  }) => {
    ...
    const paystubDocument = page
      .locator("text=EARNINGS STATEMENT")
      .locator("..");
    await expect(paystubDocument).toHaveCSS("font-family", /Arial/);
    await expect(paystubDocument).toHaveCSS(
      "background-color",
      "rgb(255, 255, 255)"
    ); // white

    const annualDocument = page
      .locator("text=ANNUAL WAGE AND TAX STATEMENT")
      .locator("..");
    await expect(annualDocument).toHaveCSS("font-family", /Times/);
    await expect(annualDocument).toHaveCSS(
      "background-color",
      "rgb(255, 255, 255)"
    ); // white
  });

  test("should maintain document formatting when emulating print", async ({
    page,
  }) => {
    // Emulate print media
    await page.emulateMedia({ media: "print" });
  ...
  test("should maintain accessibility standards for PDF buttons", async ({
    page,
  }) => {
    const pdfButtons = page.locator('button:has-text("📄 Save as PDF")');
