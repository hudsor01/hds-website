...
describe('Paystub Calculator Hooks', () => {
	describe('usePaystubCalculations', () => {
		it('should calculate pay periods correctly', () => {
			...
			const updateFn = mockSetPaystubData.mock.calls[0]?.[0] as (
				data: PaystubData
			) => PaystubData
			...
			if (updateFn) {
				...
				expect(updatedData.totals.netPay).toBeLessThan(
					updatedData.totals.grossPay
				)
			}
		})
	...
	describe('usePaystubValidation', () => {
		it('should validate required fields', () => {
			...
			expect(result.current.formErrors.employeeName).toBe(
				'Employee name must be at least 2 characters'
			)
			expect(result.current.formErrors.hourlyRate).toBe(
				'Hourly rate must be greater than $0'
			)
			expect(result.current.formErrors.hoursPerPeriod).toBe(
				'Hours per pay period must be greater than 0'
			)
		})
