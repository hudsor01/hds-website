...
test.describe('Contact Form', () => {
	...
	test('should allow selecting options from Radix Select dropdowns', async ({
		page
	}) => {
	...
	test('should successfully submit form with valid data', async ({
		...
		// Integration test: needs a live backend (Resend + DB) and sends a real
		// email. Skipped unless E2E_LIVE_BACKEND is set so the default suite/CI
		// stays green without provisioned secrets.
		test.skip(
			!process.env.E2E_LIVE_BACKEND,
			'Requires live backend (Resend + DB). Set E2E_LIVE_BACKEND=1 to run.'
		)
		...
		await page.fill(
			'#message',
			'This is an automated E2E journey test. Please disregard.'
		)
		...
		await expect(
			page.locator(
				'text=Form submitted successfully, we will get back to you soon'
			)
		).toBeVisible()
		await expect(page.locator('form')).not.toBeVisible()

		logger.complete(
			'Form submitted successfully - success message and h2 visible, form hidden'
		)
	})

	test('should show pending state while submitting', async ({
		page
	}, testInfo: TestInfo) => {
		const logger = createTestLogger(testInfo.title)
		...
		try {
			...
			if (
				buttonText?.includes('Submitting') ||
				buttonText?.includes('...') ||
				isDisabled
			) {
				...
				const hasResult = await page
					.locator('text=/Thank you|error/i')
					.isVisible()
					.catch(() => false)
				if (hasResult) {
	...
	test('should allow resending after successful submission', async ({
		page
	}, testInfo: TestInfo) => {
		const logger = createTestLogger(testInfo.title)
		...
		if (successVisible) {
			...
			const resetButton = page
				.locator('button')
				.filter({ hasText: /another|reset|new/i })
				.first()
	...
	test('should preserve form data on validation error', async ({ page }) => {
		...
		await expect(page.locator('#message')).toHaveValue(
			'Test message that should be preserved.'
		)
	})
